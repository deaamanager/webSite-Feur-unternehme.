...
async function Service({ params: { slug } }: Props) {
  ...
  return (
    <article className=" px-3 md:px-14 pb-28 overflow-x-hidden ">
          ...
          <Carousel loop>
            {service?.sliderImages.map((silderImage) => {
              return (
                <div
                  key={service._id}
                  className="relative w-full left-0 right-0 h-80 flex-[0_0_100%] filter drop-shadow-lg backdrop-filter backdrop-grayscale backdrop-blur-md backdrop-contrast-200 "
                >
                  <Image
      ...
      <hr className="border-[#ffbf00] my-8 " />
      <div></div>
      <section className="space-y-2 border-none  hover:scale-105 transition  duration-700 ease-out">
        <div className="relative h-full  min-h-full flex flex-col md:flex-row">
          <div className="absolute top-2 w-full h-full opacity-50 p-10 blur-sm rounded-lg">
            <Image
              className="object-cover object-center mx-auto rounded-2xl shadow-black shadow-xl"
          ...
          <section className="relative p-5 drop w-full h-full pt-20">
            <div className="flex flex-col justify-between md:px-14 lg:px-20  gap-y-5">
              ...
              <div className="space-y-16 pb-10">
                <p className="text-[#111] text-base font-semibold">
                  {service?.description}
                </p>
              </div>
            </div>
