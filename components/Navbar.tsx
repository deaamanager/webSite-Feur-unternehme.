...
function Navbar({firmaInfo}: Props) {
  ...
  return (
    <div className="fixed w-full left-0 right-0 z-50 overflow-x-hidden ">
      <nav className={` w-full ${show && 'bg-[rgba(0,0,0,0.7)] rounded-b-2xl  shadow'}  `}>
        <div className=" justify-between px-2 mx-auto lg:max-w-7xl md:items-center md:flex md:px-8">
          <div>
          {firmaInfo.map((firmaInfo) => (
            <div key={firmaInfo._id} className="flex items-center justify-between py-3 md:py-5 md:block">
              <Link href="/" className="flex items-start flex-col gapy-y-2">
                <motion.img
                  ...
                  className="h-24 transition-transform duration-300 ease-out " alt="SYR Services GmbH Logo" />
              </Link>
              <motion.div
          ...
          <div>
            ...
            >
              <ul className={`items-center justify-center space-y-8 md:flex md:space-x-6 md:space-y-0 text-[#ffbf00]/70 bg-[rgba(0,0,0,0.4)] font-bold text-base md:bg-inherit 
        rounded-3xl text-center  py-5
            ${show && 'bg-inherit'}`}>
                ...
                <motion.li
                  initial={{ opacity: 0, x: 50 }}
                  whileInView={{ opacity: 1, x: 0 }}
                  transition={{ duration: 0.6, delay: 0.7, type: 'spring',stiffness:50 }}
                  className="">
                  <Link href="/#ueber_uns">
                    Über uns
                  </Link>
                ...
                <motion.li
                  initial={{ opacity: 0, x: 50 }}
                  whileInView={{ opacity: 1, x: 0 }}
                  transition={{ duration: 0.6, delay: 1, type: 'spring',stiffness:50 }}
                  className="">
