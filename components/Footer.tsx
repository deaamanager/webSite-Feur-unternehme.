  ...
  const handleSubmit = async (e: any) => {
    ...
    try {
      ...
      if (req.status === 250) {
        setResponseMessage({
          isSuccessful: true,
          message: "Vielen Dank für Ihre Nachricht,Wir Kontaktieren Sie bald.",
        });
      }
    } catch (e) {
      setResponseMessage({
    ...
    <div className="relative bottom-0 self-end bg-black/90 pt-10  rounded-3xl  shadow-xl overflow-x-hidden ">
      ...
      <footer className="relative h-full rounded-lg shadow bg-black bg-opacity-30 m-4 ">
        <div className="w-full max-w-screen-xl mx-auto p-4 md:py-8 ">
            ...
            >
              ...
              <label className=" flex flex-row space-x-3 py-5 px-10 md:pl-18  text-white text-sm font-semibold">
                <input
                  ...
                  id="checkbox"
                />
                <span className="text-[10px] font-[600] md:text-sm ">
                  Hiermit stimme ich
                  <Link href="/Privce">
                    <span className="text-[#ffbf00]/40 font-[700] cursor-pointer underline hover:text-[#ffbf00]">
                      den Datenschutzhinweisen
                    </span>{" "}
                  </Link>{" "}
                  sowie der Erfassung personenbezogener Daten zur Bearbeitung
                  meiner Anfrage zu.
                </span>
              </label>
              {/*submit button */}
              <button
                type="submit"
                value="submit"
                className="bg-[#ffbf00] h-8 w-[80%] rounded-xl text-black "
              >
                Submit
              </button>
              {responseMessage && (
          ...
          <span className="block text-[11px] md:text-sm  pb-3 text-gray-200  text-center">
            Copyright © 2023 - All right reserved by Deaa Aldin Alawad
          </span>
        </div>
      </footer>
    </div>
